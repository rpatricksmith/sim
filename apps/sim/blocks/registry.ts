import { A2ABlock, A2ABlockMeta } from '@/blocks/blocks/a2a'
import { AgentBlock, AgentBlockMeta } from '@/blocks/blocks/agent'
...
import { ApiBlock, ApiBlockMeta } from '@/blocks/blocks/api'
...
import { ImageGeneratorBlock, ImageGeneratorBlockMeta } from '@/blocks/blocks/image_generator'
...
import { McpBlock, McpBlockMeta } from '@/blocks/blocks/mcp'
...
import { SearchBlock, SearchBlockMeta } from '@/blocks/blocks/search'
...
import { SttBlock, SttBlockMeta, SttV2Block } from '@/blocks/blocks/stt'
...
import { TranslateBlock, TranslateBlockMeta } from '@/blocks/blocks/translate'
import { TrelloBlock, TrelloBlockMeta } from '@/blocks/blocks/trello'
import { TtsBlock, TtsBlockMeta } from '@/blocks/blocks/tts'
...
import {
  VideoGeneratorBlock,
  VideoGeneratorBlockMeta,
  VideoGeneratorV2Block,
} from '@/blocks/blocks/video_generator'
import { VisionBlock, VisionBlockMeta, VisionV2Block } from '@/blocks/blocks/vision'
...
/**
 * All block presentation/catalog metas keyed by block type. Sibling to
 * `BLOCK_REGISTRY`; pulled from the same block files so the two stay in
 * lockstep without a separate registry to maintain.
 */
const BLOCK_META_REGISTRY: Record<string, BlockMeta> = {
  a2a: A2ABlockMeta,
  agent: AgentBlockMeta,
  ...
  image_generator: ImageGeneratorBlockMeta,
  imap: ImapBlockMeta,
  ...
  video_generator: VideoGeneratorBlockMeta,
  vision: VisionBlockMeta,
...
export function getBlock(type: string): BlockConfig | undefined {
  // Normalize dashes to underscores (some external sources use either form).
  return BLOCK_REGISTRY[type] ?? BLOCK_REGISTRY[type.replace(/-/g, '_')]
}
...
export function getLatestBlock(baseType: string): BlockConfig | undefined {
  ...
  const versionedKeys = Object.keys(BLOCK_REGISTRY).filter((key) =>
    new RegExp(`^${normalized}_v(\\d+)$`).test(key)
  )
  if (versionedKeys.length > 0) {
    ...
    return BLOCK_REGISTRY[sorted[0]!]
  }
...
export function isValidBlockType(type: string): type is string {
  return type in BLOCK_REGISTRY || type.replace(/-/g, '_') in BLOCK_REGISTRY
}

/** Get the presentation/catalog meta for a single block type. */
export function getBlockMeta(type: string): BlockMeta | undefined {
  return BLOCK_META_REGISTRY[type] ?? BLOCK_META_REGISTRY[type.replace(/-/g, '_')]
}
...
export function getTemplatesForBlock(type: string): ScopedBlockTemplate[] {
  ...
  for (const [ownerType, meta] of Object.entries(BLOCK_META_REGISTRY)) {
    ...
    const ownerBase = ownerType.replace(/_v\d+$/, '')
    ...
    for (const template of meta.templates) {
      ...
      for (const also of template.alsoIntegrations ?? []) {
        const alsoBase = also.replace(/_v\d+$/, '')
...
/** @deprecated Kept for backward compatibility with callers that expect a raw map. */
